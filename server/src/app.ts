...
export async function buildApp(options: AppOptions): Promise<FastifyInstance> {
  ...
  const houseAuditScope = async (houseId: string, scope: Scope) => {
    const house = await repository.getHouse(houseId, scope);
    return house ? { propertyCompanyId: house.propertyCompanyId || null, communityId: house.communityId || null } : null;
  };
  ...
  app.post('/api/v1/buildings', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const input = parse(buildingSchema, request.body); const community = await repository.getCommunity(input.communityId, auth.scope); if (!community) throw notFound('小区不存在或无权访问'); const row = await repository.createBuilding(input); await repository.audit({ action: 'BUILDING_CREATED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building', resourceId: row.id, requestId: request.id, afterData: { code: row.code, name: row.name } }); return ok(reply, row); });
  app.patch('/api/v1/buildings/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); const before = await repository.getBuilding(id, auth.scope); if (!before) throw notFound(); const community = await repository.getCommunity(before.communityId, auth.scope); if (!community) throw notFound(); const row = await repository.updateBuilding(id, parse(buildingPatchSchema, request.body), auth.scope); await repository.audit({ action: 'BUILDING_UPDATED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building', resourceId: id, requestId: request.id, beforeData: { code: before.code, name: before.name }, afterData: row }); return ok(reply, row); });
  app.post('/api/v1/buildings/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); const before = await repository.getBuilding(id, auth.scope); if (!before) throw notFound(); const community = await repository.getCommunity(before.communityId, auth.scope); if (!community) throw notFound(); const row = await repository.disableBuilding(id, auth.scope); await repository.audit({ action: 'BUILDING_DISABLED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building', resourceId: id, requestId: request.id }); return ok(reply, row); });
  ...
  app.post('/api/v1/buildings/:id/units', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const building = await repository.getBuilding(pathId(request), auth.scope); if (!building) throw notFound(); const community = await repository.getCommunity(building.communityId, auth.scope); if (!community) throw notFound(); const row = await repository.createBuildingUnit({ ...parse(unitSchema, request.body), buildingId: building.id }); await repository.audit({ action: 'BUILDING_UNIT_CREATED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building_unit', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
  ...
  app.patch('/api/v1/units/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); const before = await repository.getBuildingUnit(id, auth.scope); if (!before) throw notFound(); const building = await repository.getBuilding(before.buildingId, auth.scope); if (!building) throw notFound(); const community = await repository.getCommunity(building.communityId, auth.scope); if (!community) throw notFound(); const row = await repository.updateBuildingUnit(id, parse(unitPatchSchema, request.body), auth.scope); await repository.audit({ action: 'BUILDING_UNIT_UPDATED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building_unit', resourceId: id, requestId: request.id, beforeData: { code: before.code, name: before.name }, afterData: row }); return ok(reply, row); });
  app.post('/api/v1/units/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); const before = await repository.getBuildingUnit(id, auth.scope); if (!before) throw notFound(); const building = await repository.getBuilding(before.buildingId, auth.scope); if (!building) throw notFound(); const community = await repository.getCommunity(building.communityId, auth.scope); if (!community) throw notFound(); const row = await repository.disableBuildingUnit(id, auth.scope); await repository.audit({ action: 'BUILDING_UNIT_DISABLED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'building_unit', resourceId: id, requestId: request.id }); return ok(reply, row); });
  ...
  app.post('/api/v1/houses', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house:write'); const input = parse(houseSchema, request.body); const building = await repository.getBuilding(input.buildingId, auth.scope); if (!building) throw notFound('楼栋不存在或无权访问'); const community = await repository.getCommunity(building.communityId, auth.scope); if (!community) throw notFound(); if (input.buildingUnitId) { const unit = await repository.getBuildingUnit(input.buildingUnitId, auth.scope); if (!unit || unit.buildingId !== building.id) throw badRequest('单元不属于该楼栋'); } const row = await repository.createHouse(input); await repository.audit({ action: 'HOUSE_CREATED', actorUserId: auth.user.id, propertyCompanyId: community.propertyCompanyId, communityId: community.id, resourceType: 'house', resourceId: row.id, requestId: request.id, afterData: { code: row.code, buildingId: row.buildingId, buildingUnitId: row.buildingUnitId } }); return ok(reply, await repository.getHouse(row.id, auth.scope)); });
  app.patch('/api/v1/houses/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house:write'); const id = pathId(request); const before = await repository.getHouse(id, auth.scope); if (!before) throw notFound(); const input = parse(housePatchSchema, request.body); if (input.buildingUnitId && input.buildingUnitId !== before.buildingUnitId) { const unit = await repository.getBuildingUnit(input.buildingUnitId, auth.scope); if (!unit || unit.buildingId !== before.buildingId) throw badRequest('单元不属于该楼栋'); } const row = await repository.updateHouse(id, input, auth.scope); await repository.audit({ action: 'HOUSE_UPDATED', actorUserId: auth.user.id, propertyCompanyId: before.propertyCompanyId, communityId: before.communityId, resourceType: 'house', resourceId: id, requestId: request.id, beforeData: { code: before.code }, afterData: row }); return ok(reply, row); });
  app.post('/api/v1/houses/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house:write'); const id = pathId(request); const before = await repository.getHouse(id, auth.scope); if (!before) throw notFound(); const row = await repository.disableHouse(id, auth.scope); await repository.audit({ action: 'HOUSE_DISABLED', actorUserId: auth.user.id, propertyCompanyId: before.propertyCompanyId, communityId: before.communityId, resourceType: 'house', resourceId: id, requestId: request.id }); return ok(reply, row); });
  ...
  app.post('/api/v1/people', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:write'); if (!auth.scope.platform && !auth.scope.companyWide) throw forbidden('小区范围用户必须通过房屋住户入口创建客户'); const input = parse(personSchema, request.body); requireCompany(auth.scope, input.propertyCompanyId); const row = await repository.createPerson(input); await repository.audit({ action: 'PERSON_CREATED', actorUserId: auth.user.id, propertyCompanyId: row.propertyCompanyId, resourceType: 'person', resourceId: row.id, requestId: request.id, afterData: { name: row.name, phone: row.phone ? 'REDACTED' : null } }); return ok(reply, { ...row, phone: undefined, maskedPhone: row.phone ? `${row.phone.slice(0, 3)}****${row.phone.slice(-4)}` : null }); });
  app.patch('/api/v1/people/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:write'); const id = pathId(request); const before = await repository.getPerson(id, auth.scope); if (!before) throw notFound(); const auditScope = await personAuditScope(before, auth.scope); const row = await repository.updatePerson(id, parse(personPatchSchema, request.body), auth.scope); await repository.audit({ action: 'PERSON_UPDATED', actorUserId: auth.user.id, ...auditScope, resourceType: 'person', resourceId: id, requestId: request.id, beforeData: { name: before.name }, afterData: row ? { name: row.name, phone: row.phone ? 'REDACTED' : null } : undefined }); return ok(reply, row ? { ...row, phone: undefined, maskedPhone: row.phone ? `${row.phone.slice(0, 3)}****${row.phone.slice(-4)}` : null } : row); });
  app.post('/api/v1/people/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:write'); const id = pathId(request); const before = await repository.getPerson(id, auth.scope); if (!before) throw notFound(); const auditScope = await personAuditScope(before, auth.scope); const row = await repository.disablePerson(id, auth.scope); await repository.audit({ action: 'PERSON_DISABLED', actorUserId: auth.user.id, ...auditScope, resourceType: 'person', resourceId: id, requestId: request.id }); return ok(reply, row); });
  ...
  app.post('/api/v1/houses/:id/relationships', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house_relation:write'); const house = await repository.getHouse(pathId(request), auth.scope); if (!house) throw notFound(); const input = parse(relationSchema, request.body); const person = await repository.getPerson(input.personId, auth.scope); if (!person || person.propertyCompanyId !== house.propertyCompanyId) throw forbidden('客户不属于当前物业公司或小区'); const row = await repository.createHouseRelationship({ houseId: house.id, ...input }); await repository.audit({ action: 'HOUSE_RELATION_CREATED', actorUserId: auth.user.id, propertyCompanyId: house.propertyCompanyId, communityId: house.communityId, resourceType: 'house_relationship', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
  ...
  app.patch('/api/v1/house-relationships/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house_relation:write'); const row = await repository.updateHouseRelationship(pathId(request), parse(relationPatchSchema, request.body), auth.scope); if (!row) throw notFound(); const auditScope = await houseAuditScope(row.houseId, auth.scope); await repository.audit({ action: 'HOUSE_RELATION_UPDATED', actorUserId: auth.user.id, ...auditScope, resourceType: 'house_relationship', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
  app.post('/api/v1/house-relationships/:id/end', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house_relation:write'); const body = z.object({ endDate: z.string().date() }).parse(request.body); const row = await repository.endHouseRelationship(pathId(request), body.endDate, auth.scope); if (!row) throw notFound(); const auditScope = await houseAuditScope(row.houseId, auth.scope); await repository.audit({ action: 'HOUSE_RELATION_ENDED', actorUserId: auth.user.id, ...auditScope, resourceType: 'house_relationship', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
  app.post('/api/v1/house-relationships/:id/verify', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'house_relation:write'); const body = parse(verifyRelationSchema, request.body); const row = await repository.verifyHouseRelationship(pathId(request), body.status, body.note || null, auth.user.id, auth.scope); if (!row) throw notFound(); const auditScope = await houseAuditScope(row.houseId, auth.scope); await repository.audit({ action: body.status === 'VERIFIED' ? 'HOUSE_RELATION_VERIFIED' : 'HOUSE_RELATION_REJECTED', actorUserId: auth.user.id, ...auditScope, resourceType: 'house_relationship', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
