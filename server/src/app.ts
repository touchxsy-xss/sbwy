...
const houseSchema = z.object({ buildingId: z.string().uuid(), buildingUnitId: z.string().uuid().optional().nullable(), code: z.string().trim().min(1).max(40), floor: z.number().int().optional().nullable(), buildingArea: z.string().regex(/^\d+(\.\d{1,2})?$/).optional().nullable(), usableArea: z.string().regex(/^\d+(\.\d{1,2})?$/).optional().nullable(), displayName: z.string().trim().max(120).optional().nullable(), legacyCode: z.string().trim().max(120).optional().nullable() });
...
function propertyFilters(request: FastifyRequest) {
  ...
  return { page: query.page ? Number(query.page) : undefined, pageSize: query.pageSize ? Number(query.pageSize) : undefined, communityId: query.communityId, buildingId: query.buildingId, unitId: query.unitId, houseId: query.houseId, keyword: query.keyword, relationshipType: query.relationshipType as any, status: query.status };
}
...
export async function buildApp(options: AppOptions): Promise<FastifyInstance> {
  ...
  app.post('/api/v1/buildings/:id/units', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const building = await repository.getBuilding(pathId(request), auth.scope); if (!building) throw notFound(); const row = await repository.createBuildingUnit({ ...parse(unitSchema, request.body), buildingId: building.id }); await repository.audit({ action: 'BUILDING_UNIT_CREATED', actorUserId: auth.user.id, communityId: building.communityId, resourceType: 'building_unit', resourceId: row.id, requestId: request.id }); return ok(reply, row); });
  ...
  app.patch('/api/v1/units/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); const before = await repository.getBuildingUnit(id, auth.scope); if (!before) throw notFound(); const row = await repository.updateBuildingUnit(id, parse(unitPatchSchema, request.body), auth.scope); await repository.audit({ action: 'BUILDING_UNIT_UPDATED', actorUserId: auth.user.id, resourceType: 'building_unit', resourceId: id, requestId: request.id, beforeData: { code: before.code, name: before.name }, afterData: row }); return ok(reply, row); });
  app.post('/api/v1/units/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'building:write'); const id = pathId(request); if (!(await repository.getBuildingUnit(id, auth.scope))) throw notFound(); const row = await repository.disableBuildingUnit(id, auth.scope); await repository.audit({ action: 'BUILDING_UNIT_DISABLED', actorUserId: auth.user.id, resourceType: 'building_unit', resourceId: id, requestId: request.id }); return ok(reply, row); });
  ...
  app.patch('/api/v1/people/:id', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:write'); const id = pathId(request); const before = await repository.getPerson(id, auth.scope, { includeHistory: true }); if (!before) throw notFound(); const row = await repository.updatePerson(id, parse(personPatchSchema, request.body), auth.scope); await repository.audit({ action: 'PERSON_UPDATED', actorUserId: auth.user.id, propertyCompanyId: before.propertyCompanyId, resourceType: 'person', resourceId: id, requestId: request.id, beforeData: { name: before.name }, afterData: row ? { name: row.name, phone: row.phone ? 'REDACTED' : null } : undefined }); return ok(reply, row ? { ...row, phone: undefined, maskedPhone: row.phone ? `${row.phone.slice(0, 3)}****${row.phone.slice(-4)}` : null } : row); });
  app.post('/api/v1/people/:id/disable', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:write'); const id = pathId(request); const before = await repository.getPerson(id, auth.scope, { includeHistory: true }); if (!before) throw notFound(); const row = await repository.disablePerson(id, auth.scope); await repository.audit({ action: 'PERSON_DISABLED', actorUserId: auth.user.id, propertyCompanyId: before.propertyCompanyId, resourceType: 'person', resourceId: id, requestId: request.id }); return ok(reply, row); });
  app.get('/api/v1/people/:id/contact', async (request, reply) => { const auth = requireAuth(request); requirePermission(auth.scope, 'person:phone:read'); const id = pathId(request); const row = await repository.getPersonContact(id, auth.scope); if (!row) throw notFound(); await repository.audit({ action: 'PERSON_PHONE_VIEWED', actorUserId: auth.user.id, resourceType: 'person', resourceId: id, requestId: request.id }); return ok(reply, row); });
