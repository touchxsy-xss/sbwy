  ...
  const repository: Repository = {
    ...
    async listEmployees(scope) {
      return allEmployees().filter(item => {
        if (!visibleCompany(scope, item.membership.propertyCompanyId)) return false;
        if (scope.platform || scope.companyWide) return true;
        return state.assignments.some(assignment => assignment.userId === item.user.id && !assignment.revokedAt && assignment.communityId && scope.communityIds.includes(assignment.communityId));
      });
    },
    async getEmployee(id, scope) { return (await repository.listEmployees(scope)).find(item => item.id === id) || null; },
    ...
    async listHouses(_scope, filters = {}) { return { items: [], page: filters.page || 1, pageSize: filters.pageSize || 20, total: 0 }; }, async getHouse() { return null; }, async createHouse() { throw new Error('Phase 2 property repository requires PostgreSQL'); }, async updateHouse() { return null; }, async disableHouse() { return null; },
    async listPeople(_scope, filters = {}) { return { items: [], page: filters.page || 1, pageSize: filters.pageSize || 20, total: 0 }; }, async getPerson() { return null; }, async createPerson() { throw new Error('Phase 2 property repository requires PostgreSQL'); }, async updatePerson() { return null; }, async disablePerson() { return null; }, async getPersonContact() { return null; },
    ...
    async listWorkOrders(_scope, filters = {}) { return { items: [], page: filters.page || 1, pageSize: filters.pageSize || 20, total: 0 }; }, async getWorkOrder() { return null; }, async createWorkOrder() { throw new Error('Phase 2B work order repository requires PostgreSQL'); }, async assignWorkOrder() { return null; }, async transitionWorkOrder() { return null; }, async confirmWorkOrderCompletion() { return null; }, async requestWorkOrderRework() { return null; }, async getWorkOrderReview() { return null; }, async createWorkOrderReview() { return null; }, async listWorkOrderEvents() { return []; }
  };
  return repository;
}
