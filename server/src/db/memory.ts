...
export function createMemoryRepository(seed: Partial<MemoryState> = {}): Repository {
  ...
  const repository: Repository = {
    ...
    async listWorkOrders(_scope, filters = {}) { return { items: [], page: filters.page || 1, pageSize: filters.pageSize || 20, total: 0 }; }, async getWorkOrder() { return null; }, async createWorkOrder() { throw new Error('Phase 2B work order repository requires PostgreSQL'); }, async assignWorkOrder() { return null; }, async transitionWorkOrder() { return null; }, async listWorkOrderEvents() { return []; }
  };
  return repository;
}
